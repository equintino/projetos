...
function chamadosAbertos(){	
	$aberto=Conexao::consultaChamado2('chamados',$where="status=1");
	$jan=0;$fev=0;$mar=0;$abr=0;$mai=0;$jun=0;
	$jul=0;$ago=0;$set=0;$out=0;$nov=0;$dez=0;
	foreach($aberto as $key2=>$item2){
		$i=date("m",$item2['abertura']);
			switch($i){
				case 01:
				$jan=$jan+$i;
				break;	
...
function chamadosPendentes(){	
	...
	$jan=0;$fev=0;$mar=0;$abr=0;$mai=0;$jun=0;
	$jul=0;$ago=0;$set=0;$out=0;$nov=0;$dez=0;
	foreach($pendentes as $key2=>$item2){
		$i=date("m",$item2['hinicio']);
			switch($i){
				case 01:
				$jan=$jan+$i;
				break;	
...
$mesesAberto=chamadosAbertos();
$mesesPendente=chamadosPendentes();

$tabela="chamados";
$fechado=Conexao::linhas2Bd($tabela,$where="status=5");
$aberto=Conexao::linhas2Bd($tabela,$where="status=1");
$pendente=Conexao::linhas2Bd($tabela,$where="status=2 and status=3");
//echo($fechado);	exit;

$data = array(
  array($mes1[1], $mesesAberto[$mes1[1]], $mesesFechado[$mes1[1]], $mesesPendente[$mes1[1]]), array($mes1[2], $mesesAberto[$mes1[2]], $mesesFechado[$mes1[2]], $mesesPendente[$mes1[2]]), array($mes1[3], $mesesAberto[$mes1[3]], $mesesFechado[$mes1[3]], $mesesPendente[$mes1[3]]),
  array($mes1[4], $mesesAberto[$mes1[4]], $mesesFechado[$mes1[4]], $mesesPendente[$mes1[4]]), array($mes1[5],  $mesesAberto[$mes1[5]], $mesesFechado[$mes1[5]], $mesesPendente[$mes1[5]]), array($mes1[6],  $mesesAberto[$mes1[6]], $mesesFechado[$mes1[6]], $mesesPendente[$mes1[6]]),
  array($mes1[7], $mesesAberto[$mes1[7]], $mesesFechado[$mes1[7]], $mesesPendente[$mes1[7]]), array($mes1[8], $mesesAberto[$mes1[8]], $mesesFechado[$mes1[8]], $mesesPendente[$mes1[8]]), array($mes1[9], $mesesAberto[$mes1[9]], $mesesFechado[$mes1[9]], $mesesPendente[$mes1[9]]),
  array($mes1[10], $mesesAberto[$mes1[10]], $mesesFechado[$mes1[10]], $mesesPendente[$mes1[10]]), array($mes1[11], $mesesAberto[$mes1[11]], $mesesFechado[$mes1[11]], $mesesPendente[$mes1[11]]), array($mes1[12], $mesesAberto[$mes1[12]], $mesesFechado[$mes1[12]], $mesesPendente[$mes1[12]]),
);
...
#Titulo do grafico
$plot->SetTitle('Chamados Abertos e Resolvidos');
# Legenda, nesse caso serao tres pq o array possui 3 valores que serao apresentados
$plot->SetLegend(array('Chamados Abertos','Resolvidos', 'Pendentes'));
